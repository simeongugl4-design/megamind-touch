...
const liveBrainData = [
  ...
  { label: "Synapses", value: "100T", unit: "" },
  { label: "Signal Speed", value: "120", unit: "m/s" },
  { label: "Cortex Temp", value: "37.2", unit: "°C" },
  { label: "Alpha Wave", value: "10.4", unit: "Hz" },
  { label: "Beta Wave", value: "22.1", unit: "Hz" },
];
        ...
        prev.map((d) => ({
          ...
          value:
            d.label === "Neurons"
              ...
              : d.label === "Synapses"
              ? "100T"
              : d.label === "Signal Speed"
              ? (118 + Math.random() * 4).toFixed(1)
              : d.label === "Cortex Temp"
              ? (36.8 + Math.random() * 0.8).toFixed(1)
              : d.label === "Alpha Wave"
              ? (9 + Math.random() * 3).toFixed(1)
      ...
      {scanning && (
        <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 z-30">
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
            {dataValues.map((s) => (
              <div key={s.label} className="px-1.5 sm:px-2 py-1 rounded bg-background/70 backdrop-blur-md border border-primary/20">
                <p className="font-mono text-[7px] sm:text-[8px] text-muted-foreground uppercase">{s.label}</p>
                <p className="font-orbitron text-[10px] sm:text-[11px] font-bold text-primary">
